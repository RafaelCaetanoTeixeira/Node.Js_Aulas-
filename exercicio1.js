...
app.get('/site/:nomeSite', (req, res) => {
  ...
  if (!nomeSite) {
    return next('Por favor, digite um nome de site (ex: google.com)');
  }
...
//Rota cria arquivo e aloca na pasta Arquivo
...
app.get('/listaArquivos', (req, res) => {
  ...
  fs.readdir(PASTA_ARQUIVOS, (err, arquivos) => {
    ...
    arquivosTxt.forEach((arquivo) => {
      listaHTML += `<li>${arquivo} - <a href="/leArquivo/${arquivo.replace(
        '.txt',
        ''
      )}">Ler</a></li>`;
    });
